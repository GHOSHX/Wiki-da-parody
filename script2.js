...
function loadImage(event, element, type) {
    ...
    reader.onload = function(e) {
        ...
        if (type === 'character') {
            element.imgSrc = imageUrl;
            updateImageInCache(imageUrl, `character-${element.id}`);
            updateCharacterImageInDOM(element.id, imageUrl);
        } else if (type === 'inspiration') {
            element.inspirationImgSrc = imageUrl;
            updateImageInCache(imageUrl, `inspiration-${element.id}`);
            updateInspirationImageInDOM(element.id, imageUrl);
        } else if (type === 'synopsis') {
            element.imgSrc = imageUrl;
            updateImageInCache(imageUrl, `synopsis-${element.id}`);
            updateSynopsisImageInDOM(element.id, imageUrl);
        }
    };
    reader.readAsDataURL(file);
}

function updateImageInCache(imageUrl, cacheKey) {
    caches.open('image-cache-v1').then(cache => {
        cache.put(cacheKey, new Response(imageUrl));
    });
}

function updateCharacterImageInDOM(id, imageUrl) {
    const row = document.querySelector(`tr[data-index="${id}"]`);
    row.querySelector('.character-img').src = imageUrl;
}

function updateInspirationImageInDOM(id, imageUrl) {
    const row = document.querySelector(`tr[data-index="${id}"]`);
    row.querySelector('.inspiration-img').src = imageUrl;
}

function updateSynopsisImageInDOM(id, imageUrl) {
    const imgElement = document.querySelector(`img[data-index="${id}"]`);
    ...
    if (savedElements) {
        savedElements.forEach(element => {
            if (element.text) {
                ...
                fetchImageFromCache(`synopsis-${element.id}`).then(imgSrc => {
...
function loadTableState() {
    ...
    if (savedCharacters) {
        savedCharacters.forEach(character => {
            const template = document.getElementById('Character-template').content.cloneNode(true);
            fetchImageFromCache(`character-${character.id}`).then(imgSrc => {
                character.imgSrc = imgSrc;
...
function fetchImageFromCache(cacheKey) {
    return caches.open('image-cache-v1').then(cache => {
        return cache.match(cacheKey).then(response => {
            return response ? response.blob().then(blob => URL.createObjectURL(blob)) : null;
        });
    });
}

function saveState() {
    localStorage.setItem('title', document.querySelector('.title').textContent);
