...
function loadImage(event, element, type) {
    ...
    reader.onload = async function(e) {
        ...
        if (type === 'character' || type === 'inspiration') {
            await saveImageToCache(imageSrc, element.id, type);
...
async function saveImageToCache(imageSrc, id, type) {
    const cache = await caches.open('image-cache');
    const response = await fetch(imageSrc);
    await cache.put(`${type}-${id}`, response);
}

async function getImageFromCache(id, type) {
    const cache = await caches.open('image-cache');
    const response = await cache.match(`${type}-${id}`);
    if (response) {
        return response.url;
    }
    return 'https://via.placeholder.com/100'; // Default image if not found in cache
}
...
async function loadTableState() {
    ...
    if (savedCharacters) {
        for (const character of savedCharacters) {
            const template = document.getElementById('Character-template').content.cloneNode(true);
            await updateRow(template, character);
    ...
    if (savedElements) {
        for (const element of savedElements) {
            if (element.text) {
                ...
                const template = document.getElementById('synopsis-img-template').content.cloneNode(true);
                await updateImage(template, element);
