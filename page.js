...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('bold-text-btn').addEventListener('click', styleText);
...
function styleText() {
    ...
    if (start !== end) {
        ...
        const beforeText = currentTextArea.value.substring(0, start);
        const afterText = currentTextArea.value.substring(end);
        
        const newText = beforeText + '<b>' + selectedText + '</b>' + afterText;
        currentTextArea.value = newText;
        
        currentTextArea.selectionStart = currentTextArea.selectionEnd = start + selectedText.length + 7;
